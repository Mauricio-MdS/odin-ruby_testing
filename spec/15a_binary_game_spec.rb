...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        valid_input = 3
        expect(game_verify.verify_input(min, max, valid_input)).to eq(valid_input)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game_verify.instance_variable_get(:@maximum)
        invalid_input = 32
        expect(game_verify.verify_input(min, max, invalid_input)).to be_nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:big_game) { described_class.new(100, 600) }
      it 'returns 9' do
        max = big_game.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    it 'increases guess_count by one' do
      initial_guess = game_display.instance_variable_get(:@guess_count)
      expect { game_display.display_turn_order(binary_search_turn) }.to change { game_display.instance_variable_get(:@guess_count) }.to(initial_guess + 1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search_turn).to receive(:make_guess)
      game_display.display_turn_order(binary_search_turn)
    end
